...
<body>
	...
	<form action="ModifyOk" method="post">
		...
		<%--ID를 <input type="text" name="id" readonly 라고 써도 됨. --%>
		<%--readonly는 읽기 전용. --%>
		ID :
		<%=strID%>
		<br> PW : <input type="password" name="pw"
			...
			%>
		</select> <input type="text" name="phone2" value=<%=p2%>> <input
			type="text" name="phone3" value=<%=p3%>>
		<%
			if (strGender.equals("man")) {
		%>
		<input type="radio" name="gender" value="man" checked>남자 <input
			type="radio" name="gender" value="woman">여자
		<%
			} else {
		%>
		<input type="radio" name="gender" value="man">남자 <input
			type="radio" name="gender" value="woman" checked>여자
		<%
			}
		%>

		<input type="submit" value="정보수정" /><br> <input type="reset"
			value="취소" />
