...
//폼3 User_ID 중복 체크
/*        public static void Id_dupl_check(string user_id)
        {
            string sqlcommand = "";
            sqlcommand = "select User_id, COUNT(*) from PCRoom group by @p1 HAVING COUNT(*)=1;";
            try
            {
                ConnectDB();
                SqlCommand cmd = new SqlCommand();
            ...
            {
                System.Windows.Forms.MessageBox.Show(e.Message+"id_dupl_check");
            }
            finally
            {
                conn.Close();
