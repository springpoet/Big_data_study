...
class Shape2{
	...
	// <생성자 선언부>
	// 원 도형 생성 시 사용
	public Shape2(String n, int a){	// 매개변수 2개: 도형 이름, 반지름
		this.name=n;
		this.a=a;
	}
	// 사각형 생성 시 사용
	public Shape2(String n, int a, int b) {	// 매개변수 3개: 도형 이름, 길이 2개
		this.name=n;
		this.a=a;
		this.b=b;
	}
	// 삼각형 생성 시 사용
	public Shape2(String n, int a, int b, Boolean t) {	// 매개변수 4개: 도형 이름, 길이 2개, 직각 여부
		this.name=n;
		this.a=a;
		this.b=b;
		this.t=t;
...
public class JavaT0812 {
	public static void main(String args[]) {
		Scanner s=new Scanner(System.in);
		// 객체 배열 선언
		Shape2 sh[]=new Shape2[3];	// 배열만 만들어진 상태
		// 객체 생성문 : 입력문 및 반복문과 조건문을 같이 활용해봄
		for(int i=0;i<3;i++) {
			String name=s.next();
			if(name.equals("원")) {	// == 사용하면 안됨
				int a=s.nextInt();
				sh[i]=new Shape2(name, a);
			}
			else if(name.equals("사각형")) {
				int a=s.nextInt();
				int b=s.nextInt();
				sh[i]=new Shape2(name, a,b);
			}
			else {	// 삼각형일 때
				int a=s.nextInt();
				int b=s.nextInt();
				Boolean t=s.nextBoolean();
				sh[i]=new Shape2(name, a, b, t);
			}
		}
